...
export const getStaticPaths: GetStaticPaths = async () =>
  await getSortedPosts().then((posts) =>
    posts.map((post) => ({ params: { id: post.id }, props: post.data })),
  );

export const GET: APIRoute<GetSortedPosts[0]["data"]> = async ({
  props: post,
  params: { id },
}) => {
  const imageBanner = (await readFile(`./src/content/${id}.dark.png`)).toString(
    "base64",
  );
  ...
  const fontSerif = await fetch(
    "https://cdn.jsdelivr.net/fontsource/fonts/eb-garamond@latest/latin-500-normal.woff",
  );

  const template = html`
    <div
      class="relative flex h-full w-full flex-col justify-center bg-black p-10"
    >
      <div
      ...
      <div class="flex h-full w-full flex-col justify-between bg-black/10 p-10">
        <span style="font-family:serif" class="text-8xl text-neutral-300"
          >${post.title}</span
        >
        <div
        ...
        >
          <div class="flex items-center text-4xl">
            <span class="mr-8">${post.pubDate.toLocaleDateString()}</span>
          ...
          <div class="flex items-center">
            ...
            />
            <span class="text-4xl font-semibold text-neutral-300"
              >mohammedsh</span
            >
          </div>
        </div>
  ...
  return satoriAstroOG({ template, width: 1200, height: 630 }).toResponse({
    satori: {
      fonts: [
        {
          name: "sans",
          data: await fontSans.arrayBuffer(),
          style: "normal",
          weight: 400,
        },
        {
          name: "serif",
          data: await fontSerif.arrayBuffer(),
          style: "normal",
          weight: 500,
        },
      ],
