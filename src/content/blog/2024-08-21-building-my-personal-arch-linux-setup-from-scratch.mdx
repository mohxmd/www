...
author: "Mohammed"
image: "./images/computer-desk-workstation.jpg"
...
> **Quick path (current approach):** this post is the full manual setup. For a fast setup, clone my dotfiles and run one command.
>
> ```bash
...
```bash
sudo pacman -S power-profiles-daemon ntfs-3g exfatprogs linux-headers bind
```
...
sudo ufw allow 8081/tcp          # Allow Metro bundler for React Native/Expo development
...
## 🌐 Setting Up Cloudflare DNS (1.1.1.1)

After getting the network connection working, I prefer to use Cloudflare DNS for faster and more private name resolution.

Cloudflare’s public DNS servers are:
...
To make them your default DNS system-wide:

<Stepper.Root>
  <Stepper.Step>
      1. Create a new file for NetworkManager:
         ```bash
   ...
   <Stepper.Step>
      2. Then edit your DNS configuration file:

      ```bash
      sudo nano /etc/resolv.conf
      ```

      Add the Cloudflare servers:
   ...
   <Stepper.Step>
      3. (Optional) Protect it from being changed:

      ```bash
      sudo chattr +i /etc/resolv.conf
      ```

      That’s it — now your Arch system will always use Cloudflare DNS for faster lookups, and NetworkManager won’t overwrite it.
      ...
      or visit [https://1.1.1.1/help](https://1.1.1.1/help).
...
1. **Download ICC Profiles**: I found a set of ICC profiles that work well for my setup. You can download them from my GitHub repository: [ICC Profiles](https://github.com/mohxmd/dotfiles/tree/main/gamma-icc).
...
![Buraq Home](./images/buraq-home.jpg "Buraq home")
...
Download my applet configuration file from [My GitHub - appletarc](https://github.com/mohxmd/dotfiles/blob/main/.config/plasma-org.kde.plasma.desktop-appletsrc) and place it inside your `.config` directory.
...
Get the KWin rules file from my GitHub: [buraq-kwin-rules.kwinrule](https://github.com/mohxmd/dotfiles/blob/main/buraq-kwin-rules.kwinrule)

**2. Add the KWin Rule:**
Go to `Settings > Window Management > Window Rules`.
Click `Import` and select the downloaded `.kwinrule` file.

> **Fun Fact**: My system name is Buraq 😄. It’s a cool name, don’t you think?
...
![GruvBox Icon Theme](./images/buraq-icon-theme.jpg "GruvBox Icon Theme")
...
![Buraq Konsole](./images/buraq-konsole.jpg "Buraq Konsole")
...
[Buraq Dotfiles](https://github.com/mohxmd/dotfiles)
